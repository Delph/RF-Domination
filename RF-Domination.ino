...
const uint64_t rAddress = 0xB00B1E50C3LL;  // Create read pipe address for the network 
const uint64_t wAddress = 0xB00B1E50C3LL;  // Set the write pipe to be the same as the read pipe
...
int RedPin = 8;               // Connect Red Button to input pin 5 
int GreenPin = 7;              // Connect Green Button to input pin 6 
...
void setup() {
  ...
  pinMode(RedPin, INPUT);     // declare pushbutton as input
  pinMode(GreenPin, INPUT);     // declare pushbutton as input
...
void loop() {
  payload.node = nodenumber;
   if ( counter > 2000) {
        ...
        winning();
        updatedisplay();
    int red = digitalRead(RedPin);  // read input value
      if (red == LOW) {            // check if the button is pressed 
...
//        payload.sensor = 0; //  Reset  Sensor Value to 0  
    ...
    int Green = digitalRead(GreenPin);  // read input value
      if (Green == LOW) {            // check if the button is pressed 
...
// ===================================================================
void setrx() {
        wirelessSPI.openReadingPipe(1,rAddress);  // Go into listening mode 
        wirelessSPI.startListening();             // Start listening for messages
